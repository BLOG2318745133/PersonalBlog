<template>
  <div class="blogs">
    <div class="content-header">
      <h1>Blog Management<small>Blogs</small></h1>
...
<!--    <div  class="m-container-small m-padded-tb-big">-->
      <div class="filter-container">
        <el-input placeholder="type the title" v-model="pagination.queryString" style="width: 200px;"
        ...
        <el-button @click="findPage()" icon="el-icon-search" class="dalfBut">Search</el-button>
        <el-button type="primary" class="butT" @click="pushPostBlogs">New Blog</el-button>
      </div>
      <div class="ui container">
        <el-table size="middle" current-row-key="id" :data="dataList" stripe highlight-current-row>
          <!--                        id,用户名，真实姓名，角色，备注，最后登录时间，创建时间-->
          <el-table-column type="index" min-width="15px" align="center" label="#"></el-table-column>
          <el-table-column prop="title" label="Title" align="center"></el-table-column>
          <el-table-column prop="typeName" label="Type" align="center"></el-table-column>
...
<!--            </template>-->
<!--          </el-table-column>-->
          <el-table-column prop="flag" label="Status" align="center"></el-table-column>
          <el-table-column prop="updateTime" label="Updated On" align="center"></el-table-column>
          <el-table-column label="Operation" align="center">
            <template slot-scope="scope">
              <el-button size="mini" type="danger" @click="handleDelete(scope.row)">Del</el-button>
              <el-button size="mini" type="primary" @click="handleUpdate(scope.row)">Edit</el-button>
...
export default {
  ...
  methods: {
    ...
    async handleDelete (data) {
      this.$confirm('Are you sure to delete this blog?', '提示', { // 确认框
        type: 'warning'
      }).then(() => {
        const blogId = data.blogId
        this.$http.delete(`/api/server/blog/deleteBlog/${blogId}`).then((res) => {
          console.log(res)
          if (res.data.flag) {
            this.findPage()
            // 弹出提示信息
    ...
    handleUpdate (data) {
      window.sessionStorage.setItem('blogIdToUpdate', data.blogId)
      this.$router.push('/postblogs')
    }
  },
  mounted () {
