<template>
  <div class="home">
    <div class="blog-banner banner">
      <h1 class="banner-title">You are browsing a blog</h1>
    </div>
    <!--中间内容-->
        ...
        <div class="ui attached positive message">
          ...
          <div class="ui middle aligned grid">
            <div class="eleven wide column">
              <ui class="list">
                <li>Author：{{dataList.nickname}}</li>
                <li>Published On {{dataList.createTime}}</li>
                <li>Available to repost if permitted by the author</li>
              </ui>
            </div>
...
<!--            </div>-->
            ...
            <div class="ui threaded comments" style="max-width: 100%">
              ...
              <div class="comment" v-for="item in dataList2" :key="item.commentId">
                ...
                <div class="content">
                  ...
                  <div class="actions" >
                    <a class="reply" @click="replyComment(item)">Reply</a>
                    <a class="reply" @click="deleteComment(item)" v-show="item.uid==uid">Del</a>
                  </div>
                </div>
                <div class="comments">
                  <div class="comment" v-for="item2 in item.children" :key="item2.commentId">
                    ...
                    <div class="content">
                      ...
                      <div class="actions" >
                        <a class="reply" @click="replyComment(item2)">Reply</a>
                        <a class="reply" @click="deleteComment(item2)" v-show="item2.uid==uid">Del</a>
                      </div>
                    </div>
                    <div class="comments">
                      ...
                      <div class="content">
                        ...
                        <div class="actions" >
                          <a class="reply" @click="replyComment(item3)">Reply</a>
                          <a class="reply" @click="deleteComment(item3)" v-show="item3.uid==uid">Del</a>
                        </div>
                      </div>
                        <div class="comments">
                          ...
                          <div class="content">
                            ...
                            <div class="actions" >
                              <a class="reply" @click="replyComment(item4)">Reply</a>
                              <a class="reply" @click="deleteComment(item4)" v-show="item4.uid==uid">Del</a>
                            </div>
                          </div>
                          </div>
                        </div>
          ...
          <div class="ui form">
            ...
            <div class="fields">
              <div class="field  m-margin-bottom-small m-mobile-wide">
                <button class="ui blue button m-mobile-wide" @click="addComment"><i class="edit icon"></i>Comment</button>
              </div>
            </div>
    ...
    <div id="toolbar" class="m-padded m-fixed m-right-bottom" >
      <div class="ui vertical icon buttons ">
        <button type="button" class="ui toc blue button" >Catalog</button>
...
export default {
  ...
  data () {
    return {
      formData: {
        blogId: '',
        content: 'Make your comment...', // 评论内容
        parentCommentId: ''
      },
      uid: '',
      user: {},
  ...
  methods: {
    ...
    toLogin () {
      ...
      // 后端指定接口验证了token的正确性
      if (!tokenStr) {
        this.$confirm('Please Login before comment', '提示', { // 确认框
          type: 'info'
        }).then(() => {
    ...
    async deleteComment (item) {
      this.$confirm('Please confirm all comments on this comment will also be deleted.', '提示', { // 确认框
        type: 'warning'
      }).then(() => {
        ...
        this.$http.delete(`/api/server/comment/${blogId}/${commentId}`).then((res) => {
          if (res.data.flag) {
            ...
            this.formData.content = 'Make your comment...'
            // 弹出提示信息
            this.$message({
      ...
      if (this.toLogin()) {
        ...
        this.$http.post('/api/server/comment/replyComment', param).then((res) => {
          if (res.data.flag) {
            ...
            this.$message.error('Fail to comment')
          }
        })
      }
    },
    replyComment (item) { // 获取被评论者的id作为父id
      this.formData.content = 'make some comment to ' + item.nickname
